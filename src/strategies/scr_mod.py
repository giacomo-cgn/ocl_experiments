...
class SCR_mod(SupervisedTemplate):
    """
    ...
    Modified to add the possibility to use augmentation on both views of the
    input batch.
    """
    ...
    def __init__(
        ...
        :param augmentations: TorchVision Compose Transformations to augment
            the input minibatch (which includes the memory buffer).
        ...
        :param use_only_transforms: If True, only use 2 different augmented views,
            otherwise concatenate original and augmented samples.
        ...
        self.use_only_transforms = use_only_transforms
    ...
    def _before_forward(self, **kwargs):
        """
        Concatenate together augmented examples.
        """
        assert self.is_training
        super()._before_forward(**kwargs)
        mb_x_augm1 = self.augmentations(self.mbatch[0])
        if self.use_only_transforms:
            mb_x_augm2 = self.augmentations(self.mbatch[0])
             # (batch_size*2, input_size)
            self.mbatch[0] = torch.cat([mb_x_augm2, mb_x_augm1], dim=0)
        else:
            # (batch_size*2, input_size)
            self.mbatch[0] = torch.cat([self.mbatch[0], mb_x_augm1], dim=0)

    def _after_forward(self, **kwargs):
        """
        Reshape the model output to have 2 views.
        """
        assert self.is_training
        super()._after_forward(**kwargs)
        assert self.mb_output.size(0) % 2 == 0
        original_batch_size = int(self.mb_output.size(0) / 2)
        examples_batch_1 = self.mb_output[:original_batch_size]
        examples_batch_2 = self.mb_output[original_batch_size:]
        # (original_batch_size, 2, output_size)
        self.mb_output = torch.stack([examples_batch_1, examples_batch_2], dim=1)
